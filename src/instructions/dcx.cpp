...
//                       DCX Impl
// =============================================================================
DCX::DCX(const std::string& instruction) : ICommand(instruction) {
    parse(Parser::SingleOperand);
}

bool DCX::execute(Model& model) {
...
void DCX::undo(Model& model) {
}

void DCX::setup_opcode_table() {
    uint8_t current_opcode = 0x0B;
